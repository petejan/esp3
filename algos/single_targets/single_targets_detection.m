function single_targets_tot=single_targets_detection(trans_obj,varargin)
...
addParameter(p,'block_len',1e7/3,@(x) x>0);
...
idx_r_tot(idx_r_tot>nanmax(idx_bot))=[];
...
mask_inter_tot=reg_obj.get_mask_from_intersection(trans_obj.Regions(idx_bad_data));
...
for ui=1:num_ite
    ...
    idx_bot=trans_obj.get_bottom_idx(idx_pings);
    idx_r=idx_r_tot;
    
    idx_r(idx_r>nanmax(idx_bot))=[];
    ...
    mask_inter=mask_inter_tot(idx_r-idx_r_tot(1)+1,idx_pings-idx_pings_tot(1)+1);
    mask(mask_inter>=1)=1;
    ...
    if ~isempty(p.Results.load_bar_comp)
        ...
        load_bar_comp.status_bar.setText('Step 1');
    end
    ...
    if ~isempty(p.Results.load_bar_comp)
        ...
        load_bar_comp.status_bar.setText('Step 2');
    end
    ...
    if ~isempty(p.Results.load_bar_comp)
        load_bar_comp.status_bar.setText('Single Target detection');
        set(p.Results.load_bar_comp.progress_bar, 'Minimum',0, 'Maximum',num_ite, 'Value',ui);
    end
    
end

single_targets_tot.nb_valid_targets=nansum(single_targets_tot.nb_valid_targets);
