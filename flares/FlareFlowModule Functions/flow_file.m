function [Info_matrix,Info_matrix2,output_filename1,...
    output_filename2]=flow_file(filename,flare,flarenames,...
    structSize,ethas,rhow,cp,gamma,kcond,rhoG0,...
...
Info_matrix=[E_mean N_mean Z_mean TSgeomean seaf_foot_radius K_c K_d total_flow_c total_flow_d];

flare_number=1:structSize;
% figure; plot (flare_number,total_flow, 'DisplayName', 'q', 'YDataSource', 'q','Marker','o'); figure(gcf)
output_filename=filename(1:(find(filename==char(46))-1));
output_filename1=strcat(output_filename,'.flow');
output_filename2=strcat(output_filename,'_no_overl.flow');

% dist_flares=NaN(factorial(structSize-1),1);
...
for i=1:structSize
    for j=1:structSize
        if (E_mean(i)~=E_mean(j)) && (N_mean(i)~=N_mean(j)) && (j>i)
            ...
            aver_radius=mean(seaf_foot_radius);
...
clc
Info_matrix2=Info_matrix;
if exist('index_weak','var')
    Info_matrix2(index_weak,:)=[];
else
    Info_matrix2=NaN;
end
% figure;plot(Info_matrix(:,1),Info_matrix(:,2),'Marker','o','Color','Blue')
% 
% if exist('index_weak','var')
%     hold on;plot(Info_matrix2(:,1),Info_matrix2(:,2),'Marker','o','Color','Red')
% end