...
%    CREATING NMEA DATA:
%
%       NMEA strings *must* be null terminated for the files to be compatible for playback
%       using the ER60 software. The null terminations run from 2 to 4 nulls and I'm
%       guessing that 1 is required and additional nulls pad to a word boundary. It is
%       not required to pad to the word boundary and the performance gain, if any, would
%       be probably be minimal (If you prove me wrong, let me know).
%
%       Echoview will read raw files where the NMEA string is not null terminated just fine.
%
%
%   COOKED NMEA OPTIONS:
%
...
%       readEKRaw to minimize the differences between the source and destination files.
...
for n = 1:2:length(varargin)
    switch lower(varargin{n})
        ...
        case 'pingrange'
            ...
            minPingNum = min(pingNumRange(1,:));
            maxPingNum = max(pingNumRange(2,:));
    ...
    switch true
        case (chan < 1000)
            ...
            if ~isempty(chanIdx)
                ...
                writerState(4,i) = pingNumRange(2,i) - pingNumRange(1,i) + 1;
