...
p.addParameter('WriteFileHtmlHeaders', true, @islogical); %Headers to allow the output file to look like a stand-alone html file.  
p.addParameter('Title', [], @ischar);
p.addParameter('FontColor', 'black', @ischar); 
p.addParameter('FontSize', 12, @isnumeric); 
p.addParameter('FontFace', 'arial', @ischar); 
p.addParameter('TextAlign', 'center', @(x)any(strcmpi(x,{'left','center','right'}))); 
p.addParameter('CellPadding', 2.5, @isnumeric); 
p.addParameter('CellSpacing', 0, @isnumeric); 
p.addParameter('BorderSize', 1, @isnumeric); 
p.addParameter('BorderColor', 'black', @ischar); 
p.addParameter('BackgroundColor', 'white', @ischar); %Background colour of the whole table
p.addParameter('Caption', [], @ischar); %This give a caption above the table
p.addParameter('DataFormatStr', '%1.1f', @ischar); %Format of numeric data, e.g. '%1.3f' for 3 decimal places
p.addParameter('FirstRowIsHeading', 0, @isnumeric); %Enter 1 to make the first row a heading style row (shown in bold font)
p.addParameter('FirstColIsHeading', 0, @isnumeric);  %Enter 1 to make the first column a heading style col (shown in bold font)
p.addParameter('RowBGColour', {}, @iscell);  %Cell vector of background colour strings: if not empty, each i-th element of this cell-vector will over-ride the table bg colour for the i-th row.  
p.addParameter('RowFontColour', {}, @iscell);  %Cell vector of background colour strings: if not empty, each i-th element of this cell-vector will over-ride the table bg colour for the i-th row.  
p.addParameter('NegativeCellFontColour', [], @ischar);  %Over-ride to font colour if the cell is numeric and negative.  
p.addParameter('TextBelowTable', [], @ischar);  %String that will be placed after the table, i.e. at the bottom. NB: must be in html format
