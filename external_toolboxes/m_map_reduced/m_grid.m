...
%              fixes implemented.
% 13/Apr/17 - added 'axes' in argument to make changes on the axes instead
%           of the gca
...
if ~IsOctave,
    ...
    patch(ax,'xdata',X(:),'ydata',Y(:),'zdata',-LARGVAL*ones(size(X(:))),'facecolor',gbackcolor,...
...
if ~isempty(xtick)
    ...
    if strcmp(gbox,'fancy'),
        if gtickdir(1)=='i',
            fancybox(ax,lg,MAP_VAR_LIST.longs,'xgrid','bottom',dpatch,gticklen,gtickstyle);
            drawticks=0;
        else
            fancybox2(ax,lg,MAP_VAR_LIST.longs,'xgrid','bottom',dpatch,gticklen,gtickstyle);
        end;
    end;
    if drawticks,
        [n,m]=size(ltx);
...
if ~isempty(ytick),
    ...
    if strcmp(gbox,'fancy'),
        if gtickdir(1)=='i',
            fancybox(ax,lt,MAP_VAR_LIST.lats,'ygrid','left',dpatch,gticklen,gtickstyle);
            drawticks=0;
        else
            fancybox2(ax,lt,MAP_VAR_LIST.lats,'ygrid','left',dpatch,gticklen,gtickstyle);
        end;
    end;
    if drawticks,
        [n,m]=size(ltx);
...
%---------------------------------------------------------
function fancybox(ax,vals,lims,gridarg1,gridarg2,dpatch,gticklen,gridarg3)
%
...
%---------------------------------------------------------
function fancybox2(ax,vals,lims,gridarg1,gridarg2,dpatch,gticklen,gridarg3);
%
%  FANCYBOX  - draws fancy outlines for either top/bottom or left/right sides,
%              depending on calling parameters.
...
[k1,k2]=size(kk);
line(ax,reshape(mean(l2x(:,kk)),k1,k2),reshape(mean(l2y(:,kk))',k1,k2),...
    repmat(LARGVAL,k1,k2),'color','k','clipping','off','tag','m_grid_fancybox2');

kk=[dpatch:(dpatch*2):size(l2x,2)-dpatch-1]'*ones(1,dpatch+1);
kk=(kk+ones(size(kk,1),1)*[1:dpatch+1])';
[k1,k2]=size(kk);
line(ax,reshape(mean(u2x(:,kk))',k1,k2),reshape(mean(u2y(:,kk))',k1,k2),...
    repmat(LARGVAL,k1,k2),'color','k','clipping','off','tag','m_grid_fancybox2');
