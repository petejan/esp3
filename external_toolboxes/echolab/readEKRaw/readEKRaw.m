...
if ~isstruct(config)
    fclose(fid);
    header = -1; data = -1; varargout = {-1};
    warning('readEKRaw:IOError', 'Probably an EK80 file: %s%s',name,ext);
    return;
end
...
if (~rData.gpsOnly)
    ...
    if (returnFreqs(1) > 0)
        %  remove transceivers we're not interested in
        [c, a, b] = setxor(returnFreqs, frequencies);
        config.transceiver(b) = [];
        CIDs(b) = [];
        %  update header data to reflect change
        config.header.transceivercount = config.header.transceivercount - length(c);
