function  [Lat,Lon] = utm2degx(xx,yy,utmzone)
% -------------------------------------------------------------------------
...
err1=find(utmzone(:,4)>'X'| utmzone(:,4)<'C', 1);
hemis_indexN= utmzone(:,4)>'M';
hemis_indexS= utmzone(:,4)<'M';
% hemis(hemis_indexN)='N';
% hemis(hemis_indexS)='S';
if isempty(err1)~=1
...
%    end
   ...
   y=yy;
   zone=zeros((n1+1),2);
   zone(1:n1,:)=utmzone(:,1:2);
   zone=zone(1:n1,:);
   zone=(zone(:,1)-48)*10+zone(:,2)-48;
%    for i=1:n1
%        zone(i)=str2double(utmzone(i,1:2));
%        i
%    end
%    zone=zone';
...
%   ablandamiento = 1 / alpha;   % 1/f
   ...
   Y(hemis_indexS) = y(hemis_indexS) - 10000000;
   
   Y(hemis_indexN) = y(hemis_indexN);
   Y=Y';
%    if hemis == 'S' || hemis == 's'
%        Y = y - 10000000;
%    else
