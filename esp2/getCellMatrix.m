...
mat = zeros(size(regCellInt.Sv_mean_lin,1),size(regCellInt.Sv_mean,2),12);
regCellInt.Layer = regCellInt.Layer+(abs(min(regCellInt.Layer(:)))+1);

mat(:,:,1) = regCellInt.Interval;            % Interval
mat(:,:,2) = pow2db_perso(regCellInt.Sv_mean_lin);             % Sv_mean
mat(:,:,3) = regCellInt.Sv_mean_lin;     % Sv_mean Linear
...
mat(:,:,13) = regCellInt.Sa_lin; 
mat(:,:,14) = regCellInt.Nb_good_pings; 


for m = 1:size(regCellInt.Sv_mean_lin,1)
    for n = 1:size(regCellInt.Sv_mean_lin,2)        
