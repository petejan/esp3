    ...
    if nb_targets_pings(i)>0&&~isempty(X_s{i-1})
        ...
        while u<=p.Results.Max_Gap_Track&&(i-1-u)>0
            ...
            if (nb_targets_pings(i-u)>0&&~isempty(X_s{i-1-u}))
                target_gate=(repmat(X_o{i},nb_targets_pings(i-1-u),1)-repmat(X_p{i-u}',1,nb_targets_pings(i))).^2./((p.Results.ExcluDistMajAxis+repmat(Z_o{i},nb_targets_pings(i-1-u),1)*tand(p.Results.MaxStdMajorAxisAngle))*(1+u*p.Results.MissedPingExpMajAxis/100)).^2+...
                    (repmat(Y_o{i},nb_targets_pings(i-1-u),1)-repmat(Y_p{i-u}',1,nb_targets_pings(i))).^2./((p.Results.ExcluDistMinAxis+repmat(Z_o{i},nb_targets_pings(i-1-u),1)*tand(p.Results.MaxStdMinorAxisAngle))*(1+u*p.Results.MissedPingExpMinAxis/100)).^2+...
