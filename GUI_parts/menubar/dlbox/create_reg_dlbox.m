...
ref={'Surface','Bottom'};
ref_idx=1;
uicontrol(reg_fig,'Style','Text','String','Reference','units','normalized','Position',[0 0.7 0.2 0.1]);
...
switch ref{ref_idx}
    case 'Surface'
     reg_fig_comp.str_surf=uicontrol(reg_fig,'Style','Text','String','Min Depth(m)','units','normalized','Position',[0 0.6 0.25 0.1]);
    case 'Bottom'
     reg_fig_comp.str_surf=uicontrol(reg_fig,'Style','Text','String','Height Above Bottom (m)','units','normalized','Position',[0 0.6 0.25 0.1]); 
end
reg_fig_comp.depth_info=uicontrol(reg_fig,'Style','edit','unit','normalized','position',[0.3 0.63 0.1 0.07],'string',5,'Tag','w');


uicontrol(reg_fig,'Style','Text','String','Cell Width','units','normalized','Position',[0 0.4 0.2 0.1]);
uicontrol(reg_fig,'Style','Text','String','Cell Height','units','normalized','Position',[0 0.3 0.2 0.1]);
...
reg_wc=trans_obj.create_WC_region('y_min',str2double(get(reg_fig_comp.depth_info,'string')),...
    ...
    'Cell_h',str2double(get(reg_fig_comp.cell_w,'string')),...
