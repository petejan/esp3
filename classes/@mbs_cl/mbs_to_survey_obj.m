...
for ifo=1:length(opt_fields)
    if strcmpi(survey_in_obj.Options.(opt_fields{ifo}),'yes')
        survey_in_obj.Options.(opt_fields{ifo})=1;
    elseif strcmpi(survey_in_obj.Options.(opt_fields{ifo}),'no')
        survey_in_obj.Options.(opt_fields{ifo})=0;
    end
end

for ifi=1:length(inf_fields)
    if strcmpi(survey_in_obj.Infos.(inf_fields{ifi}),'yes')
        survey_in_obj.Infos.(inf_fields{ifi})=1;
    elseif strcmpi(survey_in_obj.Infos.(inf_fields{ifi}),'no')
...
for isnap=1:length(snap_numbers)
   ...
   for istrat=1:length(stratum_names)
      ...
      for itrans=1:length(trans_numbers)
          ...
          for sub_i_trans=idx_trans
              i_ori=i_ori+1;
              trans_curr.OriginCrest{i_ori}=fullfile(mbs_obj.Input.crestDir{sub_i_trans},trans_curr.files{i_ori});
