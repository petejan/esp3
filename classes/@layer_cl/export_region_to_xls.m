...
output_f_def=fullfile(path_tmp,[layers_Str{1} 'reg_' active_reg.disp_str() '.xlsx']);
...
regs=regs(is);
reg_descriptors=layer_obj.Transceivers(p.Results.idx_freq).get_region_descriptors(active_reg);

for ir=1:length(idx_freq_sort)
    output_reg=layer_obj.Transceivers(ir).integrate_region_v3(regs(ir));
...
reg_descr_table = struct2table(reg_descriptors,'asarray',1);
