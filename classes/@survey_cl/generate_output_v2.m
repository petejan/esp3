...
for isn = 1:length(snaps)
    ...
    try
        ...
        if isempty(idx_lay)
            warning('Could not find layers for this transect.');
            continue;
        end
        ...
        if isempty(idx_lay)
            fprintf('    This transect was already processed.\n');
            continue;
        end
        ...
        if nb_bad_trans/nb_ping_tot > surv_in_obj.Options.BadTransThr/100
            fprintf('    Number of bad pings (%.3f%%) in this transect exceeds threshold (%d%%). Ignoring it.\n',100*nb_bad_trans/nb_ping_tot,surv_in_obj.Options.BadTransThr);
            continue;
        end
